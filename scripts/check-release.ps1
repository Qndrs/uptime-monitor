...
Assert-True ($plugin -match "\*\s+Version:\s+3\.4\.0") "Plugin header version must be 3.4.0"
Assert-True ($readme -match "Stable tag:\s+3\.4\.0") "README stable tag must be 3.4.0"
Assert-True ($readme -match "### 3\.4\.0") "README changelog must include 3.4.0"
Assert-True ($readme -notmatch "ð|â|Ÿ|Œ|§") "README still contains mojibake characters"
Assert-True ($wpReadme -match "Stable tag:\s+3\.4\.0") "readme.txt stable tag must be 3.4.0"
