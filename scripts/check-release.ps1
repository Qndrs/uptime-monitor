...
Assert-True ($plugin -match "\*\s+Version:\s+3\.1\.1") "Plugin header version must be 3.1.1"
Assert-True ($readme -match "Stable tag:\s+3\.1\.1") "README stable tag must be 3.1.1"
Assert-True ($readme -match "### 3\.1\.1") "README changelog must include 3.1.1"
Assert-True ($readme -notmatch "ð|â|Ÿ|Œ|§") "README still contains mojibake characters"
Assert-True ($wpReadme -match "Stable tag:\s+3\.1\.1") "readme.txt stable tag must be 3.1.1"
